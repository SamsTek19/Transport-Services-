ALTER TABLE bookings
  ...
  ADD CONSTRAINT bookings_return_time_valid CHECK (
    return_time IS NULL OR return_time ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$'
  ),
  ADD CONSTRAINT bookings_round_trip_return_time_required CHECK (
    round_trip = FALSE OR return_time IS NOT NULL
  ) NOT VALID;
...
DECLARE
  ...
  requested_round_trip BOOLEAN := COALESCE((booking_data->>'round_trip')::BOOLEAN, false);
BEGIN
  IF NULLIF(TRIM(booking_data->>'name'), '') IS NULL
    ...
    OR NULLIF(booking_data->>'pickup_time', '') IS NULL
    OR (requested_round_trip AND NULLIF(booking_data->>'return_time', '') IS NULL) THEN
  ...
  IF booking_data->>'pickup_date' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
    OR booking_data->>'pickup_time' !~ '^([01][0-9]|2[0-3]):[0-5][0-9]$'
    OR (NULLIF(booking_data->>'return_time', '') IS NOT NULL
      AND booking_data->>'return_time' !~ '^([01][0-9]|2[0-3]):[0-5][0-9]$') THEN
    RAISE EXCEPTION 'Please provide valid booking date and times.' USING ERRCODE = '22023';
  END IF;
  ...
  INSERT INTO bookings (
    name, email, phone, pickup_address, dropoff_address, pickup_date, pickup_time, return_time,
